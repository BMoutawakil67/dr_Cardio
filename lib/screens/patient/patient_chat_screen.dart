...
class _PatientChatScreenState extends State<PatientChatScreen> {
  ...
  void _attachPhoto() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('📷 Prendre une photo')),
    );
  }

  void _recordVoice() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('🎤 Enregistrer un message vocal')),
    );
  }
  ...
  void _showCallDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Cette fonctionnalité nécessite un abonnement Premium'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  void _showVideoCallDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Row(
          children: [
            Icon(Icons.video_call, color: AppTheme.primaryBlue),
            SizedBox(width: 12),
            Text('Appel vidéo'),
          ],
        ),
